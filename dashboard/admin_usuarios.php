    ...
    <div class="row gx-0">
      ...
      <nav id="sidebar" class="col-12 col-md-3 col-lg-2 d-none d-lg-block p-3 mr-side">
        ...
        <ul class="nav nav-pills flex-column gap-1">
          ...
          <li class="nav-item">
            <a class="nav-link active" href="admin_usuarios.php"><i class="bi bi-shield-lock"></i> Panel Administrador</a>
          </li>
        </ul>
      ...
      <main class="col-md-10  px-4">
        ...
        <div class="container-xxl py-3">
          ...
          <!-- Barra superior: búsqueda, filtros y crear -->
          <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
            <div class="input-group" style="max-width: 360px;">
              <span class="input-group-text"><i class="bi bi-search"></i></span>
              <input id="usr_q" type="search" class="form-control" placeholder="Buscar nombre, usuario, correo…">
            </div>

            <select id="usr_sede" class="form-select" style="max-width: 280px;">
              <option value="">Todas las sedes</option>
              <!-- se llena vía JS -->
            </select>

            <select id="usr_ac" class="form-select" style="max-width: 280px;">
              <option value="">Todos los AC</option>
              <!-- se llena vía JS (opcional: usuarios con rol AC) -->
            </select>

            <button class="btn btn-primary ms-auto" data-bs-toggle="modal" data-bs-target="#modalNuevoUsuario">
              <i class="bi bi-person-plus"></i> Nuevo usuario
            </button>
          </div>

          <!-- Grid 3 columnas x 10 filas (30 por página) -->
          <div id="gridUsuarios" class="row g-3"></div>

          <!-- Paginación -->
          <nav class="mt-3 d-flex justify-content-center">
            <ul id="paginacionUsuarios" class="pagination pagination-sm mb-0"></ul>
          </nav>
        </div>

        <!-- Offcanvas Ver más -->
        <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasUsuario">
          <div class="offcanvas-header">
            <h5 class="offcanvas-title">Detalle de usuario</h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas"></button>
          </div>
          <div class="offcanvas-body" id="offcanvasUsuarioBody">
            <div class="text-center py-4 text-muted">Selecciona “Ver más” en alguna tarjeta…</div>
          </div>
        </div>
        ...
        <div class="modal fade" id="modalPassword" tabindex="-1" aria-hidden="true">
          <div class="modal-dialog">
            <form id="formPassword" class="modal-content needs-validation" novalidate>
              <input type="hidden" name="usId" id="pw_usId">
              ...
              <div class="modal-body">
                ...
                <input type="password" name="pass1" id="pw_1" class="form-control" required
                  pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*\W).{8,}$"
                  title="Mínimo 8 caracteres, mayúscula, minúscula y caracter especial.">
                <div class="invalid-feedback">No cumple los requisitos.</div>

                <label class="form-label mt-3">Confirmar contraseña</label>
                <input type="password" id="pw_2" class="form-control" required>
        ...
        <div class="modal fade" id="modalRol" tabindex="-1" aria-hidden="true">
          <div class="modal-dialog">
            <form id="formRol" class="modal-content">
              <input type="hidden" name="usId" id="rol_usId">
              <div class="modal-header">
              ...
              <div class="modal-body">
                ...
                <select name="usRol" id="rol_select" class="form-select">
                  <option value="AC">AC (Admin Cliente)</option>
                  <option value="UC">UC (Usuario Cliente)</option>
                  <option value="EC">EC (Extra Cliente)</option>
                </select>
              </div>
              <div class="modal-footer">
                <button class="btn btn-secondary" data-bs-dismiss="modal" type="button">Cancelar</button>
          ...
          <div class="modal-dialog">
            <form id="formNuevoUsuario" class="modal-content">
              ...
              <div class="modal-body">
                <div class="row g-2">
                  <div class="col-md-6">
                    ...
                    <input name="usNombre" class="form-control" required>
                  </div>
                  <div class="col-md-6">
                    <label class="form-label">Apellido paterno</label>
                    <input name="usAPaterno" class="form-control" required>
                  </div>
                  <div class="col-md-6">
                    <label class="form-label">Apellido materno</label>
                    <input name="usAMaterno" class="form-control">
                  </div>
                  <div class="col-md-6">
                    <label class="form-label">Correo</label>
                    <input name="usCorreo" type="email" class="form-control" required>
                  </div>
                  <div class="col-md-6">
                    <label class="form-label">Teléfono</label>
                    <input name="usTelefono" class="form-control">
                  </div>
                  <div class="col-md-6">
                    <label class="form-label">Usuario</label>
                    <input name="usUsername" class="form-control" required>
                  </div>

                  <div class="col-md-6">
                    <label class="form-label">Rol</label>
                    <select name="usRol" id="nu_usRol" class="form-select" required>
                      <option value="UC">Usuario</option>
                      <option value="AC">Administrador</option>
                      <option value="EC">Visualizador</option>
                    </select>
                  </div>

                  <div class="col-md-6">
                    <label class="form-label">Sede</label>
                    <select name="csId" id="nu_csId" class="form-select" required>
                      <option value="">Selecciona sede…</option>
                      <!-- se llena desde JS con adm_sedes_list.php -->
                    </select>
                  </div>

                  <!-- Solo si eres MRA/MRSA y puedes crear para otros clientes:
               muestra un select con clientes; si NO, deja oculto con tu clId -->
                  <?php if (in_array($_SESSION['usRol'] ?? '', ['MRA', 'MRSA'])): ?>
                    <div class="col-12">
                      <label class="form-label">Cliente</label>
                      <select name="clId" id="nu_clId" class="form-select">
                        <!-- llena desde JS si se requiere -->
                      </select>
                    </div>
                  <?php else: ?>
                    <input type="hidden" name="clId" id="nu_clId" value="<?= $_SESSION['clId'] ?? '' ?>">
                  <?php endif; ?>
              ...
              <div class="modal-footer">
                ...
                <button id="btnNuevoUsuario" class="btn btn-primary" type="submit">Crear usuario</button>
              </div>
            </form>
          </div>
        </div>
