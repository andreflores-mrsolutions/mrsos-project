...
$stmt->close();
// ... después de obtener $ticket principal

$ticketId = (int)$ticket['tiId'];

$accesoIngenieros = [];
$sqlAcc = "SELECT taiId, usId, nombre, email, telefono,
                  curp, nss,
                  autoPlacas, autoMarca, autoModelo, autoColor,
                  celMarca, celModelo, celSerie,
                  lapMarca, lapModelo, lapSerie,
                  extraInfo, creadoEn
           FROM ticket_acceso_ingeniero
           WHERE tiId = ?
           ORDER BY creadoEn ASC";

$stAcc = $conectar->prepare($sqlAcc);
if ($stAcc) {
  $stAcc->bind_param('i', $ticketId);
  $stAcc->execute();
  $rsAcc = $stAcc->get_result();
  while ($row = $rsAcc->fetch_assoc()) {
    $accesoIngenieros[] = $row;
  }
  $stAcc->close();
}

// Incluir en la respuesta
$ticket['accesoIngenieros']   = $accesoIngenieros;
$ticket['tiAccesoFolio']      = $ticket['tiAccesoFolio'] ?? null;
$ticket['tiAccesoSoportePath'] = $ticket['tiAccesoSoportePath'] ?? null;
