  ...
  <div class="container-fluid">
    <div class="row gx-0">
      ...
      <nav id="sidebar" class="col-12 col-md-3 col-lg-2 d-none d-lg-block p-3 mr-side">
        ...
        <ul class="nav nav-pills flex-column gap-1">
          <li class="nav-item">
            <a class="nav-link active" href="misequipos.php"><i class="bi bi-cpu"></i> Mis equipos</a>
          </li>
          <li class="nav-item">
      ...
      <div class="offcanvas offcanvas-start offcanvas-xl mr-side" tabindex="-1" id="offcanvasSidebar">
        ...
        <div class="offcanvas-body pt-0">
          ...
          <ul class="nav nav-pills flex-column gap-1 mb-4">
            <li class="nav-item"><a class="nav-link active" href="misequipos.php"><i class="bi bi-cpu"></i> Mis equipos</a></li>
      ...
      <main class="col-md-10  px-4">
        ...
        <div class="main mb-4">
          <div class="d-flex align-items-center justify-content-between mb-3">
            <h5>Equipos en Póliza</h5>
            <div class="d-flex gap-2">
              <select id="filtroTipoEq" class="form-select form-select-sm" style="width:auto;">
              ...
              <button id="btnRecargarEquipos" class="btn btn-sm btn-outline-secondary">
                <i class="bi bi-arrow-clockwise"></i> Recargar
              </button>
            </div>
          </div>

          <div class="row" id="listaEquipos"></div>
        </div>

        <!-- Offcanvas Detalle Equipo (opcional) -->
        <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasEquipo">
          <div class="offcanvas-header bg-light">
            <h5 class="offcanvas-title">Detalle del Equipo</h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas"></button>
          </div>
          <div class="offcanvas-body" id="detalleEquipoBody">
            <!-- Se llena por JS -->
          </div>
        </div>
        <script>
          $(document).ready(function() {
            ...
            // Filtro por tipo
            $("#filtroTipoEq").on("change", function() {
              cargarEquiposPoliza($(this).val());
            });

            // Recargar
            $("#btnRecargarEquipos").on("click", function() {
              $("#filtroTipoEq").val("");
              cargarEquiposPoliza();
            });
          });

          function cargarEquiposPoliza(tipoFiltro = "") {
            const $grid = $("#listaEquipos");
            $grid.html(`
    ...
    `);
            ...
            $.ajax({
              ...
              success: function(response) {
                if (!response?.success) {
                  $grid.html(`<p class="text-center text-danger">${response?.error || 'No se pudo cargar.'}</p>`);
                  return;
                }

                let equipos = response.equipos || [];
                if (tipoFiltro) {
                  equipos = equipos.filter(e => (e.eqTipoEquipo || '').toLowerCase() === tipoFiltro.toLowerCase());
                }

                if (equipos.length === 0) {
                  $grid.html('<p class="text-center text-muted">No hay equipos para mostrar.</p>');
                  return;
                }

                const html = equipos.map(eq => buildEquipoCard(eq)).join('');
                $grid.html(html);
              },
              error: function() {
                $grid.html('<p class="text-center text-danger">Error al cargar los equipos.</p>');
              }
            });
          }

          function buildEquipoCard(equipo) {
            const modelo = equipo.eqModelo || '—';
            const version = equipo.eqVersion ? ` ${equipo.eqVersion}` : '';
            const marca = (equipo.maNombre || '').toLowerCase();
            const sn = equipo.peSN || '—';
            const tipo = equipo.eqTipoEquipo || 'Equipo';
            const poliza = equipo.pcTipoPoliza || '—';
            const imgPath = `../img/Equipos/${marca}/${modelo}.png`; // adapta si tu ruta cambia

            // badge por tipo de equipo
            const tipoBadgeClass = 'bg-light text-dark';
            // badge por tipo de póliza (ajusta si manejas niveles: Oro/Plata/etc.)
            const polizaBadgeClass = 'bg-primary';

            return `
    
      <div class="col-12 col-sm-6 col-md-4 col-xl-3 mb-4">
        <div class="card service-card position-relative overflow-hidden" data-eqid="${equipo.eqId}">
          
          <!-- Tipo de Equipo (esquina) -->
          ...
          <img src="${imgPath}"
               ...
               alt="${modelo}"
               onerror="this.onerror=null;this.src='https://www.xfusion.com/wp-content/uploads/2023/04/1288H-V7-mb.png';">

          <!-- Serial -->
          <div class="serial-number ms-2">
            <span class="badge" style="background-color:#cdd6ff; color:black;">${sn}</span>
          </div>
          ...
          <!-- Marca (vertical/rotado con logo) -->
          <div class="model-vertical-marca">
            <img src="../img/Marcas/${marca}.png" alt="${equipo.maNombre || ''}" style="width:120px;">
          </div>

          <!-- Chips de info -->
          <div class="eq-chips d-flex align-items-center mt-5 px-3">
            <span class="badge ${polizaBadgeClass}">Póliza: ${poliza}</span>
          </div>

          <!-- Acciones -->
          <div class="d-flex gap-2 px-3 pb-3 mt-3">
            <button class="btn btn-sm btn-outline-primary w-50"
                    data-bs-toggle="offcanvas"
                    data-bs-target="#offcanvasEquipo"
                    onclick="verDetalleEquipo(${equipo.eqId}, '${escapeHtml(modelo)}', '${escapeHtml(version)}', '${escapeHtml(sn)}', '${escapeHtml(equipo.maNombre)}', '${escapeHtml(tipo)}', '${escapeHtml(poliza)}')">
              Detalles
            </button>
            <button class="btn btn-sm btn-success w-50" onclick="iniciarTicket(${equipo.eqId})">
              Crear Ticket
            </button>
          </div>
        </div>
    ...
    `;
          ...
          // Detalle en offcanvas
          function verDetalleEquipo(eqId, modelo, version, sn, marca, tipo, poliza) {
            const body = `
      <div class="mb-3">
        <div class="text-muted small">Equipo</div>
        <div><strong>${modelo}${version || ''}</strong></div>
      </div>
      <div class="row g-3">
        <div class="col-6">
      ...
      <hr>
      <div class="d-grid">
        <button class="btn btn-success" onclick="iniciarTicket(${eqId})">Crear Ticket</button>
      </div>
    `;
            $("#detalleEquipoBody").html(body);
          }

          // Ir al flujo de nuevo ticket (ajusta la ruta si ya tienes wizard)
          function iniciarTicket(eqId) {
            // Ejemplo: redirige a tu wizard de nuevo ticket
            window.location.href = 'layout/nuevo_ticket.php?eqId=' + encodeURIComponent(eqId);
          }

          // Utilidad para evitar romper HTML al pasar strings
